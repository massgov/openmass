...
Feature: Form Page Content type
  ...
  Scenario: Formstack URLs are output correctly when using the formstack type
    Given I am viewing a "form_page" content:
      ...
      | field_form_embed:value | "<script type="text/javascript" src="http://example.com/script?nojquery=1"></script><noscript><a href="http://example.com/noscript" title="Online Form">Test</a></noscript><div><a href="http://example.com/link" title="Web Form Creator">Web Form Creator</a></div>" |
      | moderation_state       | published |
    Then I should see a script element with the source "http://example.com/script?nojquery=1&no_style_strict=1&jsonp=1"
  ...
  Scenario: Formstack URLs are output correctly when using the formstack_reload type
    Given I am viewing a "form_page" content:
      ...
      | field_form_embed:value | "<script type="text/javascript" src="http://example.com/script?nojquery=1"></script><noscript><a href="http://example.com/noscript" title="Online Form">Test</a></noscript><div><a href="http://example.com/link" title="Web Form Creator">Web Form Creator</a></div>" |
      | moderation_state       | published |
    Then I should see a script element with the source "http://example.com/script?nojquery=1&no_style_strict=1"
