...
class ListUsageController extends ControllerBase {
  ...
  /**
   * Count of published node pages linking to a target entity.
   *
   * Collapses paragraph and non-reusable block sources to their parent node,
   * then returns the count of *published* nodes (status = 1).
   *
   * @param string $entity_type
   *   Target entity type (e.g., 'node', 'media').
   * @param int|string $entity_id
   *   Target entity ID.
   *
   * @return int
   *   Number of published pages linking here.
   */
  public function countPublishedLinkingPages(string $entity_type, $entity_id): int {
    $this->loadEntity($entity_type, $entity_id);
    if (!$this->entity) {
      return 0;
    }
    ...
    // 1) Direct node sources.
    if (!empty($sources_by_type['node'])) {
      foreach ($sources_by_type['node'] as $source_id => $records) {
        $nids[] = (int) $source_id;
      }
    }

    // 2) Paragraph sources → collapse to parent node.
    if (!empty($sources_by_type['paragraph'])) {
      foreach (array_keys($sources_by_type['paragraph']) as $pid) {
        // Use same parent lookup you already rely on elsewhere.
        if ($paragraph = $this->entityTypeManager->getStorage('paragraph')->load($pid)) {
          /** @var \Drupal\node\NodeInterface|null $parent */
          $parent = \Drupal\mayflower\Helper::getParentNode($paragraph);
          if ($parent) {
            $nids[] = (int) $parent->id();
          }
        }
    ...
    // 3) Non-reusable block_content sources → collapse to parent via usage.
    if (!empty($sources_by_type['block_content'])) {
      foreach (array_keys($sources_by_type['block_content']) as $bid) {
        $block = $this->entityTypeManager->getStorage('block_content')->load($bid);
        if ($block && method_exists($block, 'isReusable') && !$block->isReusable()) {
          // Find a parent through usage (same approach as getSourceEntityLink()).
          $parents = $this->entityUsage->listSources($block, FALSE);
          $parent = reset($parents);
          if (!empty($parent['source_type']) && !empty($parent['source_id']) && $parent['source_type'] === 'node') {
            $nids[] = (int) $parent['source_id'];
          }
        }
      }
    }
