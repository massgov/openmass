...
If any filenames do not match, you will be redirected to a review screen to confirm those files manually.</p>', ['@count' => self::MAX_UPLOADS]),
    ...
    // Add accessibility status field (applies to all uploaded documents).
    $form['accessibility_status'] = [
      '#type' => 'radios',
      '#title' => $this->t('Accessibility Status (self-reported)'),
      '#description' => $this->t('Report if these documents meet <a href="https://www.mass.gov/info-details/americans-with-disabilities-act-ada-title-ii-digital-accessibility-information">accessibility standards</a>. This status will be applied to all documents in this upload. This data appears in reports that help you track and review accessibility. If you\'re unsure how to create, test, or remediate a document to make it accessible, visit the <a href="https://www.mass.gov/accessibility-training-resource-center">accessibility training resource center</a>. Questions? Sign-up for <a href="https://www.mass.gov/info-details/accessibility-office-hours">ACCESS Office Hours</a>.'),
      '#options' => [
        'verified_accessible' => $this->t('<strong>Verified accessible:</strong><br>My organization or a third-party vendor has checked the document, and it passes automated and manual <a href="https://www.mass.gov/info-details/creating-accessible-electronic-documents-and-communications#accessibility-testing-checklists">accessibility checks</a>.'),
        'unverified_or_not_accessible_yet' => $this->t('<strong>Unverified or not accessible yet:</strong><br> The document is not accessible or hasn\'t been <a href="https://www.mass.gov/info-details/creating-accessible-electronic-documents-and-communications#accessibility-testing-checklists">checked for accessibility</a> yet. I understand that my organization needs to replace this document with an accessible version as soon as possible.'),
        'exception' => $this->t('<strong>Exception:</strong><br>This document is part of a set of documents approved as an exception. Executive Department organizations must use this <a href="https://www.mass.gov/info-details/enterprise-digital-accessibility-policy-exception-request">Exception Request Process</a>. Organizations outside the Executive Branch may use their own exception process. This document must be provided in an accessible format on request. Marking a document as an exception does not remove your obligations under state and federal law.'),
      ],
      '#required' => TRUE,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $accessibility_status = $form_state->getValue('accessibility_status');
    ...
    foreach ($items as $item) {
      if (!empty($item['path']) && file_exists($item['path'])) {
        $operations[] = [
          ...
          [$item, $user->id(), $user->getDisplayName(), $accessibility_status],
        ];
      }
    }
  ...
  public static function processFileBatch($item, $uid, $username, $accessibility_status, &$context) {
    ...
    if ($is_match && $media && $old_file) {
      ...
      $copied = $file_system->copy($path, $destination_uri, FileExists::Replace);
      if (!$copied) {
