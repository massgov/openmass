      ...
      if ($node instanceof NodeInterface && $node->bundle() == 'binder') {
        ...
        if ($node->hasField($term_ref_name) && $data_type = $node->get($term_ref_name)) {
          ...
          if (!empty($items)) {
            ...
            //   $outputValue = '1. DATA TYPE: ' . $data_type->getString() . ' TYPE NAME: ' . $type_name;
            // }

            // elseif ($data_resources->isEmpty() && strpos($type_name, 'resource') !== FALSE) {
            //   // Type = Data resource + 'field_data_resource_type' = unchecked.
            //   $outputValue = '2. DATA TYPE: ' . $data_type . ' TYPE NAME: ' . $type_name;
            // }

            // if (!$items[0]->get('field_details_datatype_metatag')->isEmpty()) {
            else {
              // Anything but 'Data resource'.
              $outputValue = 'ENTITY TERM: ' . $items[0]->get('field_details_datatype_metatag')->getString() . ' DATA TYPE: ' . $type_name = $items[0]->get('name')->getString();
            }
            // $outputValue DOESN'T GET PASSED.
            // THIS CONDISTION DOESN'T GET TRUE TO PASS THE VARIABLE.
          }
          else {
            // $outputValue = "puppy";
          }
