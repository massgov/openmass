...
/**
 * Support editing configuration for pulling bigquery stats.
 *
 * @package Drupal\mass_bigquery\Form
 */
class MassBigqueryConfig extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->config('mass_bigquery.config');

    $help_text = 'The following configuration controls the timing, batch size, and content for pulling data from '
      . 'BigQuery. During a cron run between start and end, Drupal will queue up all nodes that match the types '
    ...
    $form['types'] = [
      ...
      '#description' => $this->t('Select content types that should have bigquery data pulled in.'),
      '#options' => $options,
      '#multiple' => TRUE,
      '#size' => count($options),
