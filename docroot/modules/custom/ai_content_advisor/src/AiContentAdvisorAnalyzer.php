 ...
 */
class AiContentAdvisorAnalyzer {
  ...
  /**
   * The AI Content Advisor settings.
   *
   ...
   *
   * @var \Drupal\ai_content_advisor\RenderEntityHtmlService
   */
   ...
   *   The http client.
   * @param \Drupal\ai_content_advisor\RenderEntityHtmlService $render_entity_html
   *   Service to render entity HTML.
  ...
  public function __construct(
    ...
    $this->logger = $logger->get('ai_content_advisor');
  ...
  protected function saveReport(string $report, string $prompt, string $html_analyzed, string $url = NULL, string $entity_type_id = NULL, int $entity_id = NULL, int $revision_id = NULL, string $langcode = NULL, array $options = []) {
    ...
    // Insert data into the 'ai_content_advisor' table.
    $insert_id = $this->connection->insert('ai_content_advisor')
      ->fields([
  ...
  public function getReports(int $entity_id) {
    // Query the 'ai_content_advisor' table for reports with the given nid.
    $query = $this->connection->select('ai_content_advisor', 'o')
