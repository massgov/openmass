...
/**
 * Filters by node's organization parent.
 *
 * Organization is determined by field_parent, or the NID itself in the
 * case of an org_page node.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("mass_views_node_org_parent_filter")
 */
class OrgParentFilter extends FilterPluginBase {
  ...
  public function query() {
    ...
    if ($value = $this->getValue()) {
      ...
      $join = $this->query->getJoinData('node__field_parent', $relationship);
      $join->type = 'INNER';

      // Ensure we have the tables we need.
      $nid_alias = $this->query->ensureTable('node_field_data', $this->relationship);
      $org_table_alias = $this->query->ensureTable('node__field_parent', $this->relationship, $join);

      $p1 = $this->placeholder() . '[]';
      $snippet = "$nid_alias.nid = $p1 OR $org_table_alias.field_parent_target_id = $p1";
