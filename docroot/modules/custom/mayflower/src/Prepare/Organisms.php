...
class Organisms {
   ...
   *    ]
   */
  public static function prepareActionFinder($entity, array $options = [], array $map = [], array &$cache_tags = []) {
    $see_all = NULL;
    ...
    $title_context = array_key_exists('titleContext', $options) ? $options['titleContext'] : "";
    $featured_heading = '';
    $featured_links = '';
    if (array_key_exists('featured_actions', $fields)) {
      if (Helper::isFieldPopulated($entity, $fields['featured_actions'])) {
        ...
        $featured_links = Helper::createIllustratedOrCalloutLinks($entity, $fields['featured_actions'], [], $cache_tags);
      }
    }
    ...
    if (array_key_exists('all_actions', $fields)) {
      if (Helper::isFieldPopulated($entity, $fields['all_actions'])) {
        ...
        $links = Helper::createIllustratedOrCalloutLinks($entity, $fields['all_actions'], $options, $cache_tags);
      }

      if (array_key_exists('seeAll', $options)) {
        $maxItems = array_key_exists('maxItems', $options) ? $options['maxItems'] : 6;
        if ($entity->{$fields['all_actions']}->count() > $maxItems) {
          $see_all = $options['seeAll'];
        }
      }
    }
  ...
  public static function preparePageBanner($entity, array $options = []) {
    ...
    if (array_key_exists('description', $fields)) {
      if (Helper::isFieldPopulated($entity, $fields['description'])) {
        if ($entity->field_display_short_description->value === '1') {
