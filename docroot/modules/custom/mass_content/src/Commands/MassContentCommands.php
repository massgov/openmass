...
class MassContentCommands extends DrushCommands {
   ...
   *
   * @command mass-content:migrate-feedback
   *
   * @usage mass-content:migrate-feedback-links
   */
  public function migrateFeedbackCom() {
    // 1. Log the start of the script.
    $this->logger()->info('Update nodes batch operations start');
    ...
    $batchId = 1;
    if (!empty($nids)) {
      $this->output()->writeln("Preparing operations for " . count($nids) . " nodes.");
      foreach ($nids as $nid) {
        // Prepare the operation. Here we could do other operations on nodes.
        $this->output()->writeln("Preparing batch: " . $batchId);
        $operations[] = [
          '\Drupal\mass_content\MassContentBatchManager::processFeedback',
          [
            $batchId,
            $storage->load($nid),
            t('Updating node @nid', ['@nid' => $nid]),
      ...
      $this->logger()->warning(dt('No nodes of this type @type to process', ['@type' => 'org_page']));
    }
    // 4. Create the batch.
    $batch = [
      'title' => t('Updating @num node(s)', ['@num' => $numOperations]),
