...
class MassContentCommands extends DrushCommands {
  ...
  /**
   * Regenerate image styles.
   *
   * @param string $type
   *   Type of node to update
   *   Argument provided to the drush command.
   *
   * @command mass-content:regenerate-images
   *
   * @usage mass-content:regenerate-images foo
   *   foo is the type of node to update,
   */
  public function regenerateImages(string $type = '') {
    $map = [
      "org_page" => "field_bg_wide",
      "service_page" => "field_service_bg_wide",
      "info_details" => "field_banner_image",
    ];
    if (!empty($type) && in_array($type, array_keys($map))) {
      $map = [$type => $map[$type]];
    }
    ...
    foreach ($nids as $nid) {
      ...
      if (!empty($width) && !empty($height)) {
        ...
        if (is_file($uri)) {
          ...
          $operations[] = [
            '\Drupal\mass_content\MassContentBatchManager::processImageGeneration',
            [
              $batchId,
              $storage->load($nid),
              $uri,
